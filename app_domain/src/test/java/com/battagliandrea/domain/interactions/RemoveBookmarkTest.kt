...
class RemoveBookmarkTest {
    ...
    lateinit var useCase: RemoveBookmark

    @Before
    fun setup() {
        MockitoAnnotations.initMocks(this)
        useCase = RemoveBookmark(imageRepository)
    }
    ...
    fun `SCENARIO_1`(){
        runBlocking {
            ...
            whenever(imageRepository.removeBookmark(image)).thenReturn(image)

            useCase(image)

            verify(imageRepository).removeBookmark(image)
    ...
    fun `SCENARIO_2`(){
        runBlocking {
            ...
            whenever(imageRepository.removeBookmark(image)).thenThrow(CustomException(errorCode = 3))
